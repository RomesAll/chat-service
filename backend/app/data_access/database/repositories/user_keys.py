...
from app.shared.dtos.keys import PublicKeyDtoCreate, PublicKeyDtoGet, PrivateKeyDtoCreate, PrivateKeyDtoGet
...
class PrivateKeyRepository(
    ...
    def get_by_id(self, user_id: str) -> list[PrivateKeyDtoGet]:
    ...
    def save(self, key_info: PrivateKeyDtoCreate) -> list[PrivateKeyDtoGet]:
        """Сохранение зашифрованной коллекции приватных ключей пользователя"""
        orm_object = self.model(**key_info.model_dump())
        self.session.add(orm_object)
        self.session.flush()
        stmt_get_private_keys = select(self.model).where(self.model.user_id == key_info.user_id)
        results = list(self.session.execute(stmt_get_private_keys).scalars().all())
        results.append(orm_object)
        return [self.dto_response(**res.to_dict()) for res in results]
